"""
Shared fixtures for writer integration tests.
...
def test_session_id(db_connection: Connection) -> uuid.UUID:
    ...
    Writers have a FK constraint on session_id → ingestion_log.session_id,
    so every writer test needs a valid session in the log first.
    """
    sid = uuid.uuid4()
